  ...
  <div class="wms-temp-rang">
    ...
    <div class="content">
          ...
          <a-row class="searchRow">
            <a-col :span="6">
              <a-form-model-item label="温区编码">
                <a-input v-model="searchForm.temp_range_id" placeholder="请输入" allowClear />
              </a-form-model-item>
            </a-col>
            <a-col :span="6">
              <a-form-model-item label="温区名称">
                <a-input v-model="searchForm.temp_range_name" placeholder="请输入" allowClear />
              </a-form-model-item>
            </a-col>
      ...
      <div class="tempRange-table">
        ...
        <a-table :columns="tableColumns" :data-source="tempRangeList" class="table-one" :rowKey="record => record.temp_range_id" :loading="isfetchTempRangeList" :pagination="pagination" @change="handleTableChange" @showSizeChange="onShowSizeChange">
          ...
          <template slot="action" slot-scope="record">
            ...
            <a-popconfirm v-if="tempRangeList.length" title="确认删除吗?" cancelText="取消" okText="确认" @confirm="() => deleteTempRange(record.temp_range_id)">
      ...
      <a-modal :title="title" :visible="isShowEditModal" :confirm-loading="confirmLoading" @ok="editOk" @cancel="editCancel" cancelText="取消" okText="确定" :width="700">
        <a-form-model ref="tempRangeFormRef" :model="tempRangeForm" :rules="tempRangeFormRules" :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }">
          <a-form-model-item label="温区编码" prop="temp_range_id" ref="temp_range_id" :label-col="{ span: 4 }" :wrapper-col="{ span: 18 }">
            <a-input v-model="tempRangeForm.temp_range_id" placeholder="请输入" @blur="
          () => {
            $refs.temp_range_id.onFieldBlur()
          }" />
          </a-form-model-item>
          <a-form-model-item label="温区名称" prop="temp_range_name" ref="temp_range_name" :label-col="{ span: 4 }" :wrapper-col="{ span: 18 }">
            <a-input v-model="tempRangeForm.temp_range_name" placeholder="请输入" @blur="
          () => {
          ...
          <a-form-model-item label="备注" :label-col="{ span: 4 }" :wrapper-col="{ span: 18 }">
            <a-input v-model="tempRangeForm.memo" placeholder="请输入" type="textarea" />
          </a-form-model-item>
        </a-form-model>
      </a-modal>
    </div>
