  ...
  <div class="wms-shelf" @scroll="handleScroll($event)">
    ...
    <div class="content">
          ...
          <a-row class="searchRow">
            <a-col :span="6">
              <a-form-model-item label="仓库编码">
                <a-select v-model="searchForm.house_id" placeholder="请选择"
                  @select="fetchSetTempList" allowClear>
            ...
            <a-col :span="6">
              <a-form-model-item label="温区编码">
                <a-select v-model="searchForm.temp_range_id" placeholder="请选择"
                  allowClear>
                  <a-select-option v-for="item in setTempList"
            ...
            <a-col :span="6">
              <a-form-model-item label="货架编码">
                <a-input v-model="searchForm.house_shelf_id" placeholder="请选择"
                  allowClear>
                </a-input>
              </a-form-model-item>
            </a-col>
            <a-col :span="6" v-if="showMoreSearchOptions">
              <a-form-model-item label="货架名称">
                <a-input v-model="searchForm.house_shelf_name" placeholder="请选择"
                  allowClear>
                </a-input>
              </a-form-model-item>
            </a-col>
            <a-col :span="6" v-if="showMoreSearchOptions">
              <a-form-model-item label="楼层编码">
                <a-input v-model="searchForm.floor_id" placeholder="请选择"
                  allowClear>
                </a-input>
              </a-form-model-item>
            </a-col>
      ...
      <div class="shelf-table">
        ...
        <a-table :columns="tableColumns" ref="tableRef" :data-source="shelfList"
          class="table-list" :rowKey="record => record.house_shelf_id"
          :loading="isfetchShelfList" :pagination="pagination"
          ...
          <template slot="action" slot-scope="record">
            ...
            <a-popconfirm v-if="shelfList.length" title="确认删除吗?" cancelText="取消"
              okText="确认" @confirm="() => deleteShelf(record.house_shelf_id)">
      ...
      <a-modal :title="title" :visible="isShowEditModal"
        ...
        <a-form-model ref="shelfFormRef" :model="shelfForm"
          :rules="shelfFormRules" :label-col="{ span: 8 }"
          :wrapper-col="{ span: 16 }">
          <a-row>
            <a-col :span="12">
              <a-form-model-item label="仓库编码" prop="house_id" ref="house_id">
                <a-select v-model="shelfForm.house_id" placeholder="请选择"
                  @select="fetchSetTempList" allowClear>
                  <a-select-option v-for="item in houseList"
                    :key="item.house_id" :value='item.house_id'>
                    {{ item.house_name }}
            ...
            <a-col :span="12">
              <a-form-model-item label="温区编码" prop="temp_range_id"
                ref="temp_range_id">
                <a-select v-model="shelfForm.temp_range_id" placeholder="请选择"
                  @select="handleSelect" allowClear>
                  <a-select-option v-for="item in setTempList"
                    :key="item.temp_range_id" :value='item.temp_range_id'>
          ...
          <a-row>
            <a-col :span="12">
              <a-form-model-item label="货架编码" prop="house_shelf_id"
                ref="house_shelf_id">
                <a-input v-model="shelfForm.house_shelf_id" placeholder="请输入"
                  @blur="
          () => {
            ...
            <a-col :span="12">
              <a-form-model-item label="货架名称" prop="house_shelf_name"
                ref="house_shelf_name">
                <a-input v-model="shelfForm.house_shelf_name" placeholder="请输入"
                  @blur="
          () => {
          ...
          () => {
            ...
            <a-col :span="12">
              <a-form-model-item label="货架内部编码" prop="shelf_code"
                ref="shelf_code">
          ...
          <a-row>
            <a-col :span="12">
              <a-form-model-item label="货架种类" prop="shelf_kind"
                ref="shelf_kind">
                <a-select v-model="shelfForm.shelf_kind" placeholder="请选择"
                  @select="getShelfKindInfo" allowClear>
                  <a-select-option v-for="item in shelfKindList"
                    :key="item.shelf_kind_id" :value='item.shelf_kind_id'>
          ...
          <a-row>
            <a-col :span="12">
              <a-form-model-item label="长（cm）" prop="shelf_length"
                ref="shelf_length">
                <a-input-number v-model="shelfForm.shelf_length"
          ...
          () => {
            ...
            <a-col :span="12">
              <a-form-model-item label="宽（cm）" prop="shelf_width"
                ref="shelf_width">
                <a-input-number v-model="shelfForm.shelf_width"
                  placeholder="请输入" :min="0" @blur="
          () => {
          ...
          <a-row>
            <a-col :span="12">
              <a-form-model-item label="高（cm）" prop="shelf_height"
                ref="shelf_height">
                <a-input-number v-model="shelfForm.shelf_height"
                  placeholder="请输入" :min="0" @blur="
          () => {
            ...
            <a-col :span="12">
              <a-form-model-item label="货架层数" prop="shelf_rows"
                ref="shelf_rows">
                <a-input-number v-model="shelfForm.shelf_rows" placeholder="请输入"
                  :min="0" @blur="
          () => {
          ...
          <a-form-model-item label="有效" :label-col="{ span: 4 }"
            :wrapper-col="{ span: 20 }">
            <a-switch v-model="shelfForm.validate" />
          </a-form-model-item>
          <a-form-model-item label="备注" :label-col="{ span: 4 }"
            :wrapper-col="{ span: 20 }">
            <a-input v-model="shelfForm.memo" placeholder="请输入"
              type="textarea" />
          </a-form-model-item>
        </a-form-model>
      </a-modal>
