<template>
  <div class="employee-cell" @scroll="handleScroll($event)">
      ...
      <a-row class="content-row">
        <a-col :span="5">
          <a-card title="目录" class="left-card" :bordered="false">
            <a-tree v-if="cellTreeData && cellTreeData.length > 0" :load-data="onLoadChildrenData" :tree-data="cellTreeData" @select="onTreeSelect" :default-expanded-keys="['0']">
        ...
        <a-col :span="19" style="height:100%">
          <a-card title="货位信息" class="right-card" :bordered="false" ref="contentRowRefs">
            <div class="employee-cell-table">
                ...
                <a-row class="toolsBtn">
                  <a-space :size="12">
                    <a-tooltip placement="top">
                      ...
                      <a-button type="primary" icon="plus-circle" @click="showEditModal()" :disabled="!isLeaf" />
                    </a-tooltip>
                    <a-tooltip placement="top">
                      <template slot="title">
                        <span>刷新</span>
              ...
              <a-table :columns="tableColumns" ref="tableRef" :data-source="cellList" class="table-list" :rowKey="record => record.shelf_cell_id" :pagination="pagination" :loading="isfetchCellList" @change="handleTableChange" @showSizeChange="onShowSizeChange" :scroll="{x:1720}">
                ...
                <template slot="action" slot-scope="record">
                  ...
                  <a-popconfirm v-if="cellList.length" title="确认删除吗?" cancelText="取消" okText="确认" @confirm="() => deleteCell(record.shelf_cell_id)">
    ...
    <a-modal :title="title" :visible="isShowEditModal" :confirm-loading="confirmLoading" @ok="editOk" @cancel="editCancel" cancelText="取消" okText="确定" :width="900">
      <a-form-model ref="cellFormRef" :model="cellForm" :rules="cellFormRules" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="货位名称" prop="shelf_cell_name" ref="shelf_cell_name">
              <a-input v-model="cellForm.shelf_cell_name" placeholder="请输入" @blur="
          () => {
          ...
          <a-col :span="12">
            <a-form-model-item label="货位编码">
              <a-input v-model="cellForm.shelf_cell_id" placeholder="自动生成" :disabled="inputDisabled" />
            </a-form-model-item>
          </a-col>
        </a-row>
        <a-row>
          ...
          <a-col :span="12">
            <a-form-model-item label="温区编码">
              <a-input v-model="cellForm.temp_range_id" :disabled="inputDisabled" />
            </a-form-model-item>
          </a-col>
        </a-row>
        <a-row>
          ...
          <a-col :span="12">
            <a-form-model-item label="货架编码">
              <a-input v-model="cellForm.house_shelf_id" :disabled="inputDisabled" />
            </a-form-model-item>
          </a-col>
        </a-row>
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="货架内部编码">
              <a-input v-model="cellForm.shelf_code" :disabled="inputDisabled" />
            </a-form-model-item>
          </a-col>
          <a-col :span="12">
            <a-form-model-item label="货架层号" prop="shelf_row_code" ref="shelf_row_code">
              <a-input v-model="cellForm.shelf_row_code" placeholder="请输入" @blur="
          () => {
        ...
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="货位内部编码" prop="shelf_cell_code" ref="shelf_cell_code">
              <a-input v-model="cellForm.shelf_cell_code" placeholder="请输入" @blur="
          () => {
        ...
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="长（cm）" prop="cell_length" ref="cell_length">
              <a-input-number v-model="cellForm.cell_length" placeholder="请输入" @blur="
          () => {
          ...
          <a-col :span="12">
            <a-form-model-item label="宽（cm）" prop="cell_width" ref="cell_width">
              <a-input-number v-model="cellForm.cell_width" placeholder="请输入" @blur="
          () => {
        ...
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="高（cm）" prop="cell_height" ref="cell_height">
              <a-input-number v-model="cellForm.cell_height" placeholder="请输入" @blur="
          () => {
