  ...
  <div class="wms-cell-box" @scroll="handleScroll($event)">
    ...
    <div class="content">
          ...
          <a-row class="searchRow">
            <a-col :span="6">
              <a-form-model-item label="仓库编码">
                <a-select v-model="searchForm.house_id" placeholder="请选择" allowClear>
      ...
      <div class="cell-box-table">
        ...
        <a-table :columns="tableColumns" ref="tableRef" :data-source="cellBoxList" class="table-list" :rowKey="record => record.shelfcell_box_id" :loading="isfetchCellBoxList" :pagination="pagination" @change="handleTableChange" @showSizeChange="onShowSizeChange">
          <!-- :scroll="{x:830}" -->
          ...
          <template slot="action" slot-scope="record">
            ...
            <a-popconfirm v-if="cellBoxList.length" title="确认删除吗?" cancelText="取消" okText="确认" @confirm="() => deleteCellBox(record.shelfcell_box_id)">
      ...
      <a-modal :title="title" :visible="isShowEditModal" :confirm-loading="confirmLoading" @ok="editOk" @cancel="editCancel" cancelText="取消" okText="确定" :width="700">
        <a-form-model ref="cellBoxFormRef" :model="cellBoxForm" :rules="cellBoxFormRules" :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }">
          <a-row>
            <a-col :span="12">
              <a-form-model-item label="仓库编码" prop="house_id">
                <a-select v-model="cellBoxForm.house_id" placeholder="请选择" :disabled="houseList.length === 0" allowClear @select="getTempRangeList">
                  <a-select-option v-for="item in houseList" :key="item.house_id" :value='item.house_id'>
                    {{ item.house_id }}
                  </a-select-option>
                </a-select>
            ...
            <a-col :span="12">
              <a-form-model-item label="温区编码" prop="temp_range_id" ref="temp_range_id">
                <a-select v-model="cellBoxForm.temp_range_id" placeholder="请选择" :disabled="tempRangeList.length === 0" @select="getFloorList" allowClear>
                  <a-select-option v-for="item in tempRangeList" :key="item.temp_range_id" :value='item.temp_range_id'>
                    {{ item.temp_range_id }}
                  </a-select-option>
                </a-select>
              </a-form-model-item>
          ...
          <a-row>
            <a-col :span="12">
              <a-form-model-item label="楼层" ref="floor_id" prop="floor_id">
                <a-select v-model="cellBoxForm.floor_id" placeholder="请选择" :disabled="floorList.length === 0" @select="getShelfList" allowClear>
                  <a-select-option v-for="item in floorList" :key="item.item_name" :value='item.item_name'>
                    {{ item.item_name }}
                  </a-select-option>
                </a-select>
              </a-form-model-item>
            ...
            <a-col :span="12">
              <a-form-model-item label="货架编码" ref="house_shelf_id" prop="house_shelf_id">
                <a-select v-model="cellBoxForm.house_shelf_id" placeholder="请选择" :disabled="shelfList.length === 0" @select="getShelfRowList" allowClear>
                  <a-select-option v-for="item in shelfList" :key="item.house_shelf_id" :value='item.house_shelf_id'>
                    {{ item.house_shelf_id }}
                  </a-select-option>
                </a-select>
              </a-form-model-item>
          ...
          <a-row>
            <a-col :span="12">
              <a-form-model-item label="货层编码" ref="shelf_row_id" prop="shelf_row_id">
                <a-select v-model="cellBoxForm.shelf_row_id" placeholder="请选择" :disabled="shelfRowList.length === 0" @select="getCellList" allowClear>
                  <a-select-option v-for="item in shelfRowList" :key="item.item_name" :value='item.item_name'>
                    {{ item.item_name }}
                  </a-select-option>
                </a-select>
              </a-form-model-item>
            ...
            <a-col :span="12">
              <a-form-model-item label="货位编码" ref="shelf_cell_id" prop="shelf_cell_id">
                <a-select v-model="cellBoxForm.shelf_cell_id" placeholder="请选择" :disabled="cellList.length === 0" allowClear>
                  <a-select-option v-for="item in cellList" :key="item.shelf_cell_id" :value='item.shelf_cell_id'>
                    {{ item.shelf_cell_id }}
                  </a-select-option>
                </a-select>
              </a-form-model-item>
            ...
            <a-col :span="12">
              <a-form-model-item label="药品箱条码" ref="box_barcode" prop="box_barcode">
                <a-auto-complete v-model="cellBoxForm.box_barcode" placeholder="请输入" :data-source="barCodeList" @select="onBoxSelect" @search="onBoxSearch">
                </a-auto-complete>
              </a-form-model-item>
            </a-col>
            <a-col :span="12">
              <a-form-model-item label="药品箱编号" ref="box_id" prop="box_id">
                <a-input v-model="cellBoxForm.box_id" placeholder="请输入" disabled />
              </a-form-model-item>
            </a-col>
          </a-row>
        </a-form-model>
...
import { addCellBox, editCellBox, deleteCellBox, getSetTempList, getFloorList, getShelfList, getShelfRowList, getRowCellList } from '@/api/wms.js'
...
export default {
  ...
  methods: {
    ...
    getCellList (shelf_row_id, type) {
      ...
      getRowCellList(params).then(res => {
        this.cellList = res
      }).catch(err => {
        this.cellList = []
        this.$message.error(err.msg)
