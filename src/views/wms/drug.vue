        ...
        <a-form-model ref="drugFormRef" :model="drugForm" :rules="drugFormRules"
          ...
          () => {
            $refs.temp_range.onFieldBlur()
                ...
                <a-select v-model="drugForm.temp_range" placeholder="请选择">
                  <a-select-option v-for="temp in tempRangeList"
                    :key="temp.temp_range_id" :value="temp.temp_range_id">
                    {{ temp.temp_range_name }}
                  </a-select-option>
                </a-select>
              </a-form-model-item>
            </a-col>
  ...
  methods: {
    ...
    getDrugList () {
      const params = {
        ...
        op_type: this.searchForm.op_type ? this.searchForm.op_type : '-1'
      }
