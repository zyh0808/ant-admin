  ...
  <div class="wms-house" @scroll="handleScroll($event)">
    ...
    <div class="content">
          ...
          <a-row class="searchRow">
            <a-col :span="6">
              <a-form-model-item label="仓库编码">
                <a-input v-model="searchForm.house_id" placeholder="请输入"
                  allowClear />
              </a-form-model-item>
            </a-col>
            <a-col :span="6">
              <a-form-model-item label="仓库名称">
                <a-input v-model="searchForm.house_name" placeholder="请输入"
                  allowClear />
              </a-form-model-item>
            </a-col>
      ...
      <div class="house-table">
        ...
        <a-table :columns="tableColumns" ref="tableRef" :data-source="houseList"
          class="table-list" :rowKey="record => record.house_id"
          :loading="isfetchHouseList" :pagination="pagination"
          ...
          <template slot="action" slot-scope="record">
            ...
            <a-button type="link" size="small"
              @click="showSettingModal(record)">
              设置温区
            </a-button>
            <a-divider type="vertical" />
            <a-popconfirm v-if="houseList.length" title="确认删除吗?" cancelText="取消"
              okText="确认" @confirm="() => deleteHouse(record.house_id)">
              <a-button type="link" size="small">
                删除
              </a-button>
            </a-popconfirm>
      ...
      <a-modal :title="title" :visible="isShowEditModal"
        ...
        <a-form-model ref="houseFormRef" :model="houseForm"
          :rules="houseFormRules" :label-col="{ span: 6 }"
          :wrapper-col="{ span: 16 }">
          <a-row>
            <a-col :span="8">
          ...
          () => {
            ...
            <a-col :span="8">
              <a-form-model-item label="仓库名称" prop="house_name"
                ref="house_name">
          ...
          () => {
            ...
            <a-col :span="8">
              <a-form-model-item label="楼层数" prop="floors" ref="floors">
                <a-input-number v-model="houseForm.floors" placeholder="请输入"
                  :min="0" @blur="
          () => {
          ...
          <a-row>
            <a-col :span="8">
              <a-form-model-item label="省份" prop="province_id"
                ref="province_id">
                <a-select v-model="houseForm.province_id"
                  @select="selectProvince" :loading="isfetchProvinceList"
                  placeholder="请选择">
            ...
            <a-col :span="8">
              <a-form-model-item label="市区" prop="city_id" ref="city_id">
                <a-select v-model="houseForm.city_id" @select="selectCity"
            ...
            <a-col :span="8">
              <a-form-model-item label="县/区" prop="county_id" ref="county_id">
                <a-select v-model="houseForm.county_id" @select="selectCounty"
                  :loading="isfetchCountyList"
                  :disabled="countyList.length === 0" placeholder="请选择">
          ...
          <a-row>
            <a-col :span="8">
              <a-form-model-item label="街道/镇" prop="town_id" ref="town_id">
                <a-select v-model="houseForm.town_id" @select="selectTown"
            ...
            <a-col :span="8">
              <a-form-model-item label="社区/村" prop="village_id"
                ref="village_id">
                <a-select v-model="houseForm.village_id" @select="selectVillage"
          ...
          <a-form-model-item label="详细地址" prop="addr" ref="addr"
            :label-col="{ span: 2 }" :wrapper-col="{ span: 16 }">
          ...
          <a-form-model-item label="备注" :label-col="{ span: 2 }"
            :wrapper-col="{ span: 16 }">
            <a-input v-model="houseForm.memo" placeholder="请输入"
              type="textarea" />
          </a-form-model-item>
        </a-form-model>
      </a-modal>
      <a-modal :title="setTitle" :visible="showTempSettingModal"
        @cancel="showTempSettingModal = false" :footer="null" :width="1000">
        <a-transfer :data-source="tempRangeList" :filter-option="(inputValue, item) => item.title.indexOf(inputValue)
          !== -1" :titles="['温区列表（未选）', '温区列表（已选）']" :target-keys="setTempList"
          :show-select-all="false" @change="onSetChange">
          ...
          " :columns="selecTableColumns" :data-source="filteredItems"
              size="small" :pagination="false" class="table-one"
              :rowKey="record => record.temp_range_id" />
          </template>
        </a-transfer>
      </a-modal>
    </div>
