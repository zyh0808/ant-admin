  ...
  <div class="wms-shelfKind" @scroll="handleScroll($event)">
    ...
    <div class="content">
          ...
          <a-row class="searchRow">
            <a-col :span="6">
              <a-form-model-item label="货架类型">
                <a-select v-model="searchForm.category" placeholder="请选择" allowClear>
            ...
            <a-col :span="6">
              <a-form-model-item label="种类名称">
                <a-input v-model="searchForm.shelf_kind_name" placeholder="请输入" allowClear />
              </a-form-model-item>
            </a-col>
      ...
      <div class="shelfKind-table">
        ...
        <a-table :columns="tableColumns" ref="tableRef" :data-source="shelfKindList" class="table-list" :rowKey="record => record.shelf_kind_id" :loading="isfetchShelfKindList" :pagination="pagination" @change="handleTableChange" @showSizeChange="onShowSizeChange">
          <!-- :scroll="{x:940}" -->
          ...
          <template slot="action" slot-scope="record">
            ...
            <a-popconfirm v-if="shelfKindList.length" title="确认删除吗?" cancelText="取消" okText="确认" @confirm="() => deleteShelfKind(record.shelf_kind_id)">
      ...
      <a-modal :title="title" :visible="isShowEditModal" :confirm-loading="confirmLoading" @ok="editOk" @cancel="editCancel" cancelText="取消" okText="确定" :width="700">
        <a-form-model ref="shelfKindFormRef" :model="shelfKindForm" :rules="shelfKindFormRules" :label-col="{ span: 4 }" :wrapper-col="{ span: 18 }">
          <a-form-model-item label="货架种类名称" prop="shelf_kind_name" ref="shelf_kind_name">
            <a-input v-model="shelfKindForm.shelf_kind_name" placeholder="请输入" @blur="
          () => {
          ...
          <a-row>
            <a-col :span="12">
              <a-form-model-item label="长（cm）" prop="shelf_length" ref="shelf_length" :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }">
                <a-input-number v-model="shelfKindForm.shelf_length" placeholder="请输入" :min="0" @blur="
          () => {
            ...
            <a-col :span="12">
              <a-form-model-item label="宽（cm）" prop="shelf_width" ref="shelf_width" :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }">
                <a-input-number v-model="shelfKindForm.shelf_width" placeholder="请输入" :min="0" @blur="
          () => {
          ...
          <a-row>
            <a-col :span="12">
              <a-form-model-item label="高（cm）" prop="shelf_height" ref="shelf_height" :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }">
                <a-input-number v-model="shelfKindForm.shelf_height" placeholder="请输入" :min="0" @blur="
          () => {
            ...
            <a-col :span="12">
              <a-form-model-item label="货架层数" prop="shelf_rows" ref="shelf_rows" :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }">
                <a-input-number v-model="shelfKindForm.shelf_rows" placeholder="请输入" :min="0" @blur="
          () => {
