<template>
  <div class="employee-dict" @scroll="handleScroll($event)">
      ...
      <a-row class="content-row">
        <a-col :span="5">
          <a-card title="分类目录" class="left-card" :bordered="false">
            <a-tree v-if="allDictTree && allDictTree.length > 0" :tree-data="allDictTree" @select="onTreeSelect" :default-expanded-keys="['TOP']">
        ...
        <a-col :span="19" style="height:100%">
          <a-card title="区域信息" class="right-card" :bordered="false" ref="contentRowRefs">
            <div class="employee-dict-table">
                ...
                <a-row class="toolsBtn">
                  <a-space :size="12">
                    <a-tooltip placement="top">
                      ...
                      <a-button type="primary" icon="plus-circle" @click="showEditModal()" :disabled="currentDictId === ''" />
                    </a-tooltip>
                    <a-tooltip placement="top">
                      <template slot="title">
                        <span>刷新</span>
              ...
              <a-table :columns="tableColumns" ref="tableRef" :data-source="dictPageList" class="table-list" :rowKey="record => record.item_id" :pagination="pagination" :loading="isfetchDictPageList" @change="handleTableChange" @showSizeChange="onShowSizeChange">
                <!-- :scroll="{x:1000}" -->
                ...
                <template slot="action" slot-scope="record">
                  ...
                  <a-popconfirm v-if="dictPageList.length" title="确认删除吗?" cancelText="取消" okText="确认" @confirm="() => deleteDict(record.item_id)">
    ...
    <a-modal :title="title" :visible="isShowEditModal" :confirm-loading="confirmLoading" @ok="editOk" @cancel="editCancel" cancelText="取消" okText="确定" :width="600">
      <a-form-model ref="dictFormRef" :model="dictForm" :rules="dictFormRules" :label-col="{ span: 5 }" :wrapper-col="{ span: 16 }">
        ...
        <a-form-model-item label="项目名称" prop="item_name" ref="item_name">
          <a-input v-model="dictForm.item_name" :max-length="100" placeholder="请输入" @blur="
          () => {
            $refs.item_name.onFieldBlur()
          }" />
        </a-form-model-item>
        <a-form-model-item label="项目值" prop="item_value" ref="item_value">
          <a-input v-model="dictForm.item_value" :max-length="100" placeholder="请输入" @blur="
          () => {
        ...
        <a-row>
          ...
          <a-col :span="12">
            <a-form-model-item label="有效" prop="validate" :label-col="{ span: 10 }" :wrapper-col="{ span: 14 }">
              <a-switch v-model="dictForm.validate" />
            </a-form-model-item>
          </a-col>
        </a-row>
        <a-form-model-item label="项目描述" prop="item_desc">
          <a-input v-model="dictForm.item_desc" type="textarea" placeholder="请输入" :max-length="300" />
        </a-form-model-item>
      </a-form-model>
    </a-modal>
  </div>
