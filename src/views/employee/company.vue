<template>
  <div class="employee-company" @scroll="handleScroll($event)">
      ...
      <a-row class="content-row">
        <a-col :span="5">
          <a-card title="目录信息" :bordered="false" class="left-card">
            <a-tree autoExpandParent show-icon v-if="companyTreeData && companyTreeData.length > 0" :tree-data="companyTreeData" default-expand-all @select="onTreeSelect">
            ...
            <div class="employee-sys-table">
              <a-row class="toolRow" type="flex" justify="space-between">
                ...
                <a-row class="toolsBtn">
                  <a-space :size="12">
                    <a-tooltip placement="top" v-action="'company.add'">
                      <template slot="title">
                        <span>添加</span>
                      </template>
                      <a-button type="primary" icon="plus-circle" @click="showEditModal()" :disabled="currentCompanyId === ''" />
                    </a-tooltip>
              ...
              <a-table :columns="tableColumns" ref="tableRef" :data-source="companyList" class="table-list" :rowKey="record => record.comp_id" :loading="isfetchCompanyList" :pagination="pagination" @change="handleTableChange" @showSizeChange="onShowSizeChange" :scroll="{x:1400}">
                ...
                <template slot="action" slot-scope="record">
                  ...
                  <a-popconfirm v-if="companyList.length" title="确认删除吗?" cancelText="取消" okText="确认" @confirm="() => deleteCompany(record.comp_id)">
    ...
    <a-modal :title="title" :visible="isShowEditModal" :confirm-loading="confirmLoading" @ok="editOk" @cancel="editCancel" cancelText="取消" okText="确定" :width="1000">
      <a-form-model ref="companyFormRef" :model="companyForm" :rules="companyFormRules" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
        <a-row>
          <a-col :span="16">
            <a-form-model-item label="上级名称" :label-col="{ span: 3 }" :wrapper-col="{ span: 16 }">
        ...
        <a-row>
          <a-col :span="8">
            <a-form-model-item label="公司名称" prop="comp_name" ref="comp_name">
              <a-input v-model="companyForm.comp_name" placeholder="请输入" :max-length="50" @blur="
          () => {
          ...
          <a-col :span="8">
            <a-form-model-item label="公司简称" prop="short_name" ref="short_name">
              <a-input v-model="companyForm.short_name" placeholder="请输入" :max-length="50" @blur="
          () => {
          ...
          <a-col :span="8">
            <a-form-model-item label="英文名称" prop="en_name" ref="en_name">
              <a-input v-model="companyForm.en_name" placeholder="请输入" :max-length="50" @blur="
          () => {
        ...
        <a-row>
          ...
          <a-col :span="8">
            <a-form-model-item label="法人代表" prop="manager" ref="manager">
              <a-input v-model="companyForm.manager" placeholder="请输入" :max-length="50" @blur="
          () => {
        ...
        <a-row>
          <a-col :span="8">
            <a-form-model-item label="开户行" prop="bank_name" ref="bank_name">
              <a-input v-model="companyForm.bank_name" placeholder="请输入" :max-length="50" @blur="
          () => {
          ...
          <a-col :span="8">
            <a-form-model-item label="银行账号" prop="bank_account" ref="bank_account">
              <a-input v-model="companyForm.bank_account" placeholder="请输入" :max-length="50" @blur="
          () => {
        ...
        <a-row>
          ...
          <a-col :span="8">
            <a-form-model-item label="传真" prop="fax" ref="fax">
              <a-input v-model="companyForm.fax" placeholder="请输入" :max-length="50" @blur="
          () => {
        ...
        <a-row>
          <a-col :span="8">
            <a-form-model-item label="省份" prop="province_id" ref="province_id">
              <a-select v-model="companyForm.province_id" @select="fetchCityList" :loading="isfetchProvinceList" placeholder="请选择">
          ...
          <a-col :span="8">
            <a-form-model-item label="市区" prop="city_id" ref="city_id">
              <a-select v-model="companyForm.city_id" @select="fetchCountyList" :loading="isfetchCityList" :disabled="cityList.length === 0" placeholder="请选择">
          ...
          <a-col :span="8">
            <a-form-model-item label="县/区" prop="county_id" ref="county_id">
              <a-select v-model="companyForm.county_id" @select="fetchTownList" :loading="isfetchCountyList" :disabled="countyList.length === 0" placeholder="请选择">
        ...
        <a-row>
          <a-col :span="8">
            <a-form-model-item label="街道/镇" prop="town_id" ref="town_id">
              <a-select v-model="companyForm.town_id" @select="fetchVillageList" :loading="isfetchTownList" :disabled="townList.length === 0" placeholder="请选择">
          ...
          <a-col :span="8">
            <a-form-model-item label="社区/村" prop="village_id" ref="village_id">
              <a-select v-model="companyForm.village_id" :loading="isfetchVillageList" :disabled="villageList.length === 0" placeholder="请选择">
        ...
        <a-row>
          <a-col :span="16">
            <a-form-model-item label="详细地址" prop="addr" ref="addr" :label-col="{ span: 3 }" :wrapper-col="{ span: 16 }">
              <a-input v-model="companyForm.addr" placeholder="请输入" :max-length="50" @blur="
          () => {
        ...
        <a-form-model-item label="经营范围" :label-col="{ span: 2 }" :wrapper-col="{ span: 16 }">
          <a-input v-model="companyForm.biz_scope" type="textarea" placeholder="请输入" :max-length="200" />
        </a-form-model-item>
        <a-form-model-item label="备注" :label-col="{ span: 2 }" :wrapper-col="{ span: 16 }">
          <a-input v-model="companyForm.memo" type="textarea" placeholder="请输入" :max-length="200" />
        </a-form-model-item>
      </a-form-model>
    </a-modal>
  </div>
