  ...
  <div class="employee-role" @scroll="handleScroll($event)">
    ...
    <div class="content">
      ...
      <div class="employee-role-table">
        ...
        <a-table :columns="tableColumns" ref="tableRef" :data-source="roleList" class="table-list" :rowKey="record =>record.role_id" :pagination="pagination" @change="handleTableChange" @showSizeChange="onShowSizeChange" :loading="isfetchRoleList">
          <!-- :scroll="{x:1050}" -->
          ...
          <template slot="action" slot-scope="record">
            ...
            <a-popconfirm v-if="roleList.length" title="确认删除吗?" cancelText="取消" okText="确认" @confirm="() => deleteRole(record.role_id)">
            ...
            <a-divider type="vertical" />
            <a-button type="link" size="small" @click="showRoleAuthModal(record)">
              角色授权
            </a-button>
          </template>
        </a-table>
    ...
    <a-modal :title="title" :visible="isShowEditModal" :confirm-loading="confirmLoading" @ok="editOk" @cancel="editCancel" cancelText="取消" okText="确定" :width="600">
      <a-form-model ref="roleFormRef" :model="roleForm" :rules="roleFormRules" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
        <a-form-model-item label="角色名称" prop="role_name" ref="role_name">
          <a-input v-model="roleForm.role_name" :max-length="50" placeholder="请输入" @blur="
          () => {
        ...
        <a-form-model-item label="角色描述">
          <a-input v-model="roleForm.memo" type="textarea" placeholder="请输入" :max-length="200" />
        </a-form-model-item>
      </a-form-model>
    </a-modal>
    <a-modal :title="roleTitle" :visible="isShowRoleModal" @cancel="roleUserCancel" :footer="null" :bodyStyle="{ height: '700px' }" :width="1200">
      <a-row style="height:100%">
        ...
        <a-col :span="18">
          ...
          !== -1" :titles="['员工列表（未选）', '员工列表（已选）']" :target-keys="roleEmpList" :show-select-all="false" @change="onRoleChange">
          ...
          " :columns="userColumns" :data-source="filteredItems" size="small" :pagination="false" class="table-one" :rowKey="record => record.user_id" :style="{ pointerEvents: listDisabled ? 'none' : null }" />
            </template>
          </a-transfer>
        </a-col>
      </a-row>
    </a-modal>
    <a-modal :title="roleAuthTitle" :visible="isShowRoleAuthModal" @cancel="isShowRoleAuthModal = false" :footer="null" :bodyStyle="{ height: '700px' }" :width="1200">
      <a-transfer class="tree-transfer" :data-source="unselectRoleAuth" :titles="['角色权限列表（未选）', '角色权限列表（已选）']" :target-keys="roleAuthTargetKeys" :render="item => item.title" :show-select-all="false" @change="roleAuthChange">
        <template slot="children" slot-scope="{ props: { direction, selectedKeys }, on: { itemSelect } }">
          <a-tree v-if="direction === 'left'? (unselectRoleAuth && unselectRoleAuth.length > 0 ) : (selectRoleAuth && selectRoleAuth.length > 0 ) " blockNode checkable defaultExpandAll :checkedKeys="[...selectedKeys, ...roleAuthTargetKeys]" :treeData="direction === 'left' ? unselectRoleAuth : selectRoleAuth" @check="
            (_, props) => {
...
export default {
  ...
  methods: {
    ...mapActions({
      fetchRolePageList: 'role/fetchRolePageList',
