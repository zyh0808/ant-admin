<template>
  <div class="employee-area" @scroll="handleScroll($event)">
      ...
      <a-row class="content-row">
        <a-col :span="5">
          <a-card title="行政区域目录" class="left-card" :bordered="false">
            <a-tree v-if="areaTreeData && areaTreeData.length > 0"
              :load-data="onLoadChildrenData" :tree-data="areaTreeData"
          ...
          <a-card title="区域信息" class="right-card" :bordered="false"
            ...
            <div class="employee-area-table">
              ...
              <a-table :columns="tableColumns" ref="tableRef"
                :data-source="areaPageList" class="table-list"
                :rowKey="record => record.area_id" :pagination="pagination"
                :loading="isfetchAreaPageList" @change="handleTableChange"
                @showSizeChange="onShowSizeChange" :scroll="{x:900}">
                ...
                <template slot="action" slot-scope="record">
                  ...
                  <a-popconfirm v-if="areaPageList.length" title="确认删除吗?"
                    cancelText="取消" okText="确认"
                    @confirm="() => deleteArea(record.area_id)">
                    <a-button type="link" size="small">
                      删除
                    </a-button>
                  </a-popconfirm>
    ...
    <a-modal :title="title" :visible="isShowEditModal"
      ...
      <a-form-model ref="areaFormRef" :model="areaForm" :rules="areaFormRules"
        ...
        <a-form-model-item label="区域编码" prop="area_id" ref="area_id">
          <a-input v-model="areaForm.area_id" placeholder="请输入" :max-length="10"
            @blur="
          () => {
        ...
        <a-form-model-item label="区域名称" prop="area_name" ref="area_name">
          <a-input v-model="areaForm.area_name" placeholder="请输入"
            :max-length="50" @blur="
          () => {
        ...
        <a-form-model-item label="简称" prop="simple_spell" ref="simple_spell">
          <a-input v-model="areaForm.simple_spell" placeholder="请输入"
            :max-length="100" @blur="
          () => {
        ...
        <a-row>
          ...
          <a-col :span="12">
            <a-form-model-item label="有效" :label-col="{ span: 10 }"
              :wrapper-col="{ span: 14 }">
  ...
  methods: {
    ...
    onLoadChildrenData (treeNode) {
      return new Promise((resolve) => {
        ...
        getAreaList({ area_id: treeNode.dataRef.key, area_name: '' }).then(data => {
