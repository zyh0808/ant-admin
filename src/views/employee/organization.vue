<template>
  <div class="employee-org" @scroll="handleScroll($event)">
    <div class="title">
      <!-- <a-page-header style="backgroundColor:#fff" title="组织机构"
        :breadcrumb="{props: {routes :pageHeaderRoutes }}" /> -->
      <pageHeader :routes="pageHeaderRoutes" :basePath="system"></pageHeader>
    </div>
    <div class="content">
      <a-row class="content-row">
        <a-col :span="5">
          <a-card title="组织机构目录" class="left-card" :bordered="false">
            <a-tree v-if="orgTreeData && orgTreeData.length > 0" show-icon :tree-data="orgTreeData" :default-expanded-keys="['0']" @select="onTreeSelect">
              <a-icon slot="switcherIcon" type="down" />
              <a-icon slot="home" type="home" />
              <a-icon slot="apartment" type="apartment" />
            </a-tree>
            <a-spin v-else size="small" />
          </a-card>
        </a-col>
        <a-col :span="19" style="height:100%">
          <a-card title="组织机构信息" class="right-card" :bordered="false" ref="contentRowRefs">
            <div class="employee-sys-table">
              <a-row class="toolRow" type="flex" justify="space-between">
                <div>
                  <h3 class="listTitle" v-if="currentOrgName">
                    {{currentOrgName}}
                  </h3>
                </div>
                <a-row class="toolsBtn">
                  <a-space :size="12">
                    <a-tooltip placement="top">
                      <template slot="title">
                        <span>添加</span>
                      </template>
                      <a-button type="primary" icon="plus-circle" @click="showEditModal()" :disabled="currentOrgId === ''" />
                    </a-tooltip>
                    <a-tooltip placement="top">
                      <template slot="title">
                        <span>刷新</span>
                      </template>
                      <a-button type="dashed" icon="reload" @click="reloadList" />
                    </a-tooltip>
                    <a-tooltip placement="top">
                      <template slot="title">
                        <span>设置</span>
                      </template>
                      <columnSelect :plainOptions="columns" @changeColumns="changeColumns">
                      </columnSelect>
                    </a-tooltip>
                  </a-space>
                </a-row>
              </a-row>
              <a-table :columns="tableColumns" ref="tableRef" :data-source="orgList" class="table-list" :rowKey="record => record.org_id" :pagination="false" :loading="isfetchOrgList">
                <!-- :scroll="{x:1050}" -->
                <!-- @change="handleTableChange" @showSizeChange="onShowSizeChange" -->
                <span slot="validate" slot-scope="validate">
                  <!-- <a-switch :checked="validate" disabled /> -->
                  <a-tag v-if="validate" color="green">有效</a-tag>
                  <a-tag v-else color="red">无效</a-tag>
                </span>
                <template slot="action" slot-scope="record">
                  <a-button type="link" size="small" @click="showEditModal(record)">
                    编辑
                  </a-button>
                  <a-divider type="vertical" />
                  <a-popconfirm v-if="orgList.length" title="确认删除吗?" cancelText="取消" okText="确认" @confirm="() => deleteOrg(record.org_id)">
                    <a-button type="link" size="small">
                      删除
                    </a-button>
                  </a-popconfirm>
                </template>
              </a-table>
              <a-table :columns="tableColumns" :class="showSticky?'sticky-table'  : ''" ref="stickyTableRef" :style="{display: 'none',width: stickyWidth + 'px'}" table-layout="fixed">
              </a-table>
            </div>
          </a-card>
        </a-col>
      </a-row>
    </div>
    <a-modal :title="title" :visible="isShowEditModal" :confirm-loading="confirmLoading" @ok="editOk" @cancel="editCancel" cancelText="取消" okText="确定" :width="1000">
      <a-form-model ref="orgFormRef" :model="orgForm" :rules="orgFormRules" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="上级机构">
              <a-input :value="orgForm.top_org_name" disabled />
            </a-form-model-item>
          </a-col>
          <a-col :span="12">
            <a-form-model-item label="所属公司">
              <a-input :value="orgForm.comp_name" disabled />
            </a-form-model-item>
          </a-col>
        </a-row>
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="机构名称" prop="org_name" ref="org_name">
              <a-input v-model="orgForm.org_name" :max-length="100" placeholder="请输入" @blur="
          () => {
            $refs.org_name.onFieldBlur()
          }" />
            </a-form-model-item>
          </a-col>
          <a-col :span="12">
            <a-form-model-item label="机构简称" prop="short_name" ref="short_name">
              <a-input v-model="orgForm.short_name" :max-length="50" placeholder="请输入" @blur="
          () => {
            $refs.short_name.onFieldBlur()
          }" />
            </a-form-model-item>
          </a-col>
        </a-row>
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="英文名称" prop="en_name" ref="en_name">
              <a-input v-model="orgForm.en_name" :max-length="50" placeholder="请输入" @blur="
          () => {
            $refs.en_name.onFieldBlur()
          }" />
            </a-form-model-item>
          </a-col>
          <a-col :span="12">
            <a-form-model-item label="机构分类">
              <a-select v-model="orgForm.category" placeholder="请选择">
                <a-select-option :value=1>
                  客服部
                </a-select-option>
                <a-select-option :value=2>
                  营销
                </a-select-option>
                <a-select-option :value=3>
                  职能
                </a-select-option>
                <a-select-option :value=4>
                  运营
                </a-select-option>
              </a-select>
            </a-form-model-item>
          </a-col>
        </a-row>
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="负责人" prop="manager" ref="manager">
              <a-input v-model="orgForm.manager" :max-length="20" placeholder="请输入" @blur="
          () => {
            $refs.manager.onFieldBlur()
          }" />
            </a-form-model-item>
          </a-col>
          <a-col :span="12">
            <a-form-model-item label="机构性质" prop="nature">
              <!-- <a-select v-model="orgForm.nature" placeholder="请选择">
                <a-select-option :value=0>
                  直营
                </a-select-option>
                <a-select-option :value=1>
                  加盟
                </a-select-option>
                <a-select-option :value=2>
                  分控
                </a-select-option>
              </a-select> -->
              <a-radio-group v-model="orgForm.nature">
                <a-radio-button :value=0>
                  直营
                </a-radio-button>
                <a-radio-button :value=1>
                  加盟
                </a-radio-button>
                <a-radio-button :value=2>
                  分控
                </a-radio-button>
              </a-radio-group>
            </a-form-model-item>
          </a-col>
        </a-row>
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="联系电话" prop="telephone" ref="telephone">
              <a-input v-model="orgForm.telephone" placeholder="请输入" @blur="
          () => {
            $refs.telephone.onFieldBlur()
          }" />
            </a-form-model-item>
          </a-col>
          <a-col :span="12">
            <a-form-model-item label="传真" prop="fax" ref="fax">
              <a-input v-model="orgForm.fax" :max-length="50" placeholder="请输入" @blur="
          () => {
            $refs.fax.onFieldBlur()
          }" />
            </a-form-model-item>
          </a-col>
        </a-row>
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="省份" prop="province_id">
              <a-select v-model="orgForm.province_id" @select="fetchCityList" :loading="isfetchProvinceList" placeholder="请选择">
                <a-select-option v-for="item in provinceList" :value="item.item_value" :key="item.item_value">
                  {{item.item_name}}
                </a-select-option>
              </a-select>
            </a-form-model-item>
          </a-col>
          <a-col :span="12">
            <a-form-model-item label="市区" prop="city_id">
              <a-select v-model="orgForm.city_id" @select="fetchCountyList" :loading="isfetchCityList" :disabled="cityList.length === 0" placeholder="请选择">
                <a-select-option v-for="item in cityList" :value="item.item_value" :key="item.item_value">
                  {{item.item_name}}
                </a-select-option>
              </a-select>
            </a-form-model-item>
          </a-col>
        </a-row>
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="县/区" prop="county_id">
              <a-select v-model="orgForm.county_id" @select="fetchTownList" :loading="isfetchCountyList" :disabled="countyList.length === 0" placeholder="请选择">
                <a-select-option v-for="item in countyList" :value="item.item_value" :key="item.item_value">
                  {{item.item_name}}
                </a-select-option>
              </a-select>
            </a-form-model-item>
          </a-col>
          <a-col :span="12">
            <a-form-model-item label="街道/镇" prop="town_id">
              <a-select v-model="orgForm.town_id" @select="fetchVillageList" :loading="isfetchTownList" :disabled="townList.length === 0" placeholder="请选择">
                <a-select-option v-for="item in townList" :value="item.item_value" :key="item.item_value">
                  {{item.item_name}}
                </a-select-option>
              </a-select>
            </a-form-model-item>
          </a-col>
        </a-row>
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="社区/村" prop="village_id">
              <a-select v-model="orgForm.village_id" :loading="isfetchVillageList" :disabled="villageList.length === 0" placeholder="请选择">
                <a-select-option v-for="item in villageList" :value="item.item_value" :key="item.item_value">
                  {{item.item_name}}
                </a-select-option>
              </a-select>
            </a-form-model-item>
          </a-col>
          <a-col :span="12">
            <a-form-model-item label="邮政编码" prop="postcode" ref="postcode">
              <a-input v-model="orgForm.postcode" :max-length="6" placeholder="请输入" @blur="
          () => {
            $refs.postcode.onFieldBlur()
          }" />
            </a-form-model-item>
          </a-col>
        </a-row>
        <a-form-model-item label="公司地址" prop="addr" ref="addr" :label-col="{ span: 3 }" :wrapper-col="{ span: 20 }">
          <a-input v-model="orgForm.addr" :max-length="100" placeholder="请输入" @blur="
          () => {
            $refs.addr.onFieldBlur()
          }" />
        </a-form-model-item>
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="排序码" prop="sort_code">
              <a-input-number v-model="orgForm.sort_code" :min="1" />
            </a-form-model-item>
          </a-col>
          <a-col :span="12">
            <a-form-model-item label="有效">
              <a-switch v-model="orgForm.validate" />
            </a-form-model-item>
          </a-col>
        </a-row>
        <a-form-model-item label="备注" :label-col="{ span: 3 }" :wrapper-col="{ span: 20 }">
          <a-input v-model="orgForm.memo" type="textarea" :max-length="200" placeholder="请输入" />
        </a-form-model-item>
      </a-form-model>
    </a-modal>
  </div>
</template>

<script>
import { mapActions, mapGetters, mapMutations } from 'vuex'
import { addOtherOrg, editOrg, deleteOrg } from '@/api/system.js'
import { checkPhone, checkPostCode } from '@/js/utils.js'
import columnSelect from '@/components/columnSelect.vue'
import pageHeader from '@/components/pageHeader.vue'
const orgForm = {
  org_name: '',
  short_name: '',
  en_name: '',
  category: null,
  manager: '',
  nature: 0,
  telephone: '',
  fax: '',
  province_id: null,
  city_id: null,
  county_id: null,
  town_id: null,
  village_id: null,
  postcode: '',
  addr: '',
  sort_code: 1,
  validate: true,
  memo: ''
}
export default {
  data () {
    const columns = [
      {
        title: '机构名称',
        dataIndex: 'org_name',
        value: 'org_name',
        width: 150,
        ellipsis: true
      },
      {
        title: '机构简称',
        dataIndex: 'short_name',
        value: 'short_name',
        width: 100,
        ellipsis: true
      },
      {
        title: '机构分类',
        dataIndex: 'category',
        value: 'category',
        width: 100,
        ellipsis: true
      },
      {
        title: '机构性质',
        dataIndex: 'nature',
        value: 'nature',
        width: 100,
        ellipsis: true
      },
      {
        title: '负责人',
        dataIndex: 'manager',
        value: 'manager',
        width: 100,
        ellipsis: true
      },
      {
        title: '联系电话',
        dataIndex: 'telephone',
        value: 'telephone',
        width: 150,
        ellipsis: true
      },
      {
        title: '排序码',
        dataIndex: 'sort_code',
        value: 'sort_code',
        width: 100,
        ellipsis: true
      },
      {
        title: '有效',
        dataIndex: 'validate',
        value: 'validate',
        width: 100,
        key: 'validate',
        scopedSlots: { customRender: 'validate' },
      },
      {
        title: '操作',
        key: 'action',
        value: 'action',
        width: 150,
        scopedSlots: { customRender: 'action' },
      }
    ]
    return {
      columns,
      tableColumns: columns,
      currentOrgId: '',
      currentOrgName: '',
      kind: 0,
      pid: '',
      title: '添加组织机构',
      isShowEditModal: false,
      confirmLoading: false,
      orgForm: {
        pid: '',
        top_org_name: '',
        comp_name: '',
        comp_id: '',
        ...orgForm
      },
      isSubmit: false,
      orgFormRules: {
        org_name: [{ required: true, message: '请输入机构名称', trigger: 'blur' }],
        short_name: [{ required: true, message: '请输入机构简称', trigger: 'blur' }],
        en_name: [{ required: true, message: '请输入英文名称', trigger: 'blur' }],
        manager: [{ required: true, message: '请输入负责人', trigger: 'blur' }],
        nature: [{ required: true, message: '请选择机构性质', trigger: 'blur' }],
        telephone: [{ required: true, message: '请输入联系电话', trigger: 'blur' }, {
          validator: this.checkPhone, trigger: 'blur'
        }],
        fax: [{ required: true, message: '请输入传真号码', trigger: 'blur' }],
        province_id: [{ required: true, message: '请选择省份', trigger: 'blur' }],
        city_id: [{ required: true, message: '请选择市区', trigger: 'blur' }],
        county_id: [{ required: true, message: '请选择县/区', trigger: 'blur' }],
        town_id: [{ required: true, message: '请选择街道/镇', trigger: 'blur' }],
        village_id: [{ required: true, message: '请选择社区/村', trigger: 'blur' }],
        postcode: [{ required: true, message: '请输入邮政编码', trigger: 'blur' }, {
          validator: this.checkPostCode, trigger: 'blur'
        }],
        addr: [{ required: true, message: '请输入详细地址', trigger: 'blur' }],
        sort_code: [{ required: true, message: '请输入排序码', trigger: 'blur' }],
      },
      isEdit: false,
      system: 'employee',
      showSticky: false,
      stickyWidth: 0
    }
  },
  components: {
    columnSelect,
    pageHeader
  },
  computed: {
    ...mapGetters({
      pageHeaderRoutes: 'common/pageHeaderRoutes',
      orgTreeData: 'orgnization/orgTreeData',
      orgList: 'orgnization/orgList',
      isfetchOrgList: 'orgnization/isfetchOrgList',
      currentOrgInfo: 'orgnization/currentOrgInfo',
      isfetchProvinceList: 'area/isfetchProvinceList',
      isfetchCityList: 'area/isfetchCityList',
      isfetchCountyList: 'area/isfetchCountyList',
      isfetchTownList: 'area/isfetchTownList',
      isfetchVillageList: 'area/isfetchVillageList',
      provinceList: 'area/provinceList',
      cityList: 'area/cityList',
      countyList: 'area/countyList',
      townList: 'area/townList',
      villageList: 'area/villageList'
    })
  },
  created () {
    const pageHeaderPoutes = [{
      path: '',
      breadcrumbName: '系统管理',
    },
    {
      path: 'employee_org',
      breadcrumbName: '组织机构'
    }]
    this.setPageHeaderRoutes(pageHeaderPoutes)
  },
  mounted () {
    this.tableList = this.$refs.tableRef.$el
    this.stickyWidth = this.tableList.clientWidth
    this.tableList.addEventListener('scroll', this.handleTableScroll, true)
    // this.stickyWidth = this.$refs.tableRef.$el.querySelector('.table-list').clientWidth
    // this.tableList = document.querySelector(".table-list")
    // this.tableList.addEventListener('scroll', this.handleTableScroll, true)
    this.fetchOrgTreeData({})
  },
  methods: {
    ...mapActions({
      fetchOrgTreeData: 'orgnization/fetchOrgTreeData',
      fetchTopOrgList: 'orgnization/fetchTopOrgList',
      fetchOtherOrgList: 'orgnization/fetchOtherOrgList',
      fetchCurrentOrgInfo: 'orgnization/fetchCurrentOrgInfo',
      fetchCurrentCompanyInfo: 'company/fetchCurrentCompanyInfo',
      fetchProvinceList: 'area/fetchProvinceList',
      fetchCityList: 'area/fetchCityList',
      fetchProvinceList: 'area/fetchProvinceList',
      fetchCityList: 'area/fetchCityList',
      fetchCountyList: 'area/fetchCountyList',
      fetchTownList: 'area/fetchTownList',
      fetchVillageList: 'area/fetchVillageList'
    }),
    ...mapMutations({
      setPageHeaderRoutes: 'common/setPageHeaderRoutes',
      resetAllData: 'orgnization/resetAllData'
    }),
    getOrgList () {
      if (this.kind === 1) { //公司
        this.fetchTopOrgList({
          comp_id: this.currentOrgId
        })
      } else { //组织机构
        this.fetchOtherOrgList({
          pid: this.currentOrgId
        })
      }
    },
    onTreeSelect (selectedKeys, e) {
      const { selected, selectedNodes, node, event } = e
      this.currentOrgId = node.value
      this.currentOrgName = node.title
      this.kind = node.$options.propsData.dataRef.kind
      this.pid = node.$options.propsData.dataRef.parent_no
      this.orgForm = Object.assign({}, this.orgForm, {
        pid: this.pid === 'TOP' && this.kind === 1 ? 'TOP' : this.currentOrgId,
        comp_id: this.kind === 1 ? node.value : '',
        top_org_name: node.title
      })
      if (this.kind === 1) {
        this.fetchCurrentCompanyInfo({
          comp_id: this.currentOrgId, success: (companyInfo) => {
            this.orgForm = Object.assign({}, this.orgForm, {
              comp_name: companyInfo.comp_name
            })
          }
        })
      } else {
        this.fetchCurrentOrgInfo({
          org_id: this.currentOrgId, success: (orgInfo) => {
            this.fetchCurrentCompanyInfo({
              comp_id: orgInfo.comp_id, success: (companyInfo) => {
                this.orgForm = Object.assign({}, this.orgForm, {
                  comp_name: companyInfo.comp_name,
                  comp_id: orgInfo.comp_id
                })
              }
            })
          }
        })
      }
      this.getOrgList()
    },
    showEditModal (orgInfo) {
      this.isShowEditModal = true
      this.fetchProvinceList({})
      if (orgInfo) {
        this.isEdit = true
        this.title = '编辑组织机构'
        this.orgForm = Object.assign(this.orgForm, orgInfo, {})
      }
    },
    editOk () {
      this.$refs.orgFormRef.validate(valid => {
        if (valid) {
          if (this.isEdit) {
            this.editOrg()
          } else {
            this.addOrg()
          }
        } else {
          return false
        }
      })
    },
    addOrg () {
      this.confirmLoading = true
      const orgForm = Object.assign(this.orgForm, { is_disabled: this.orgForm.validate ? 0 : 1 })
      const params = {
        ...orgForm,
        success: () => {
          this.$message.success('操作成功')
          this.getOrgList()
          this.editCancel()
        }
      }
      addOtherOrg(params).then(res => {
        if (params.success) {
          params.success()
        }
      }).catch(err => {
        this.$message.error(err.msg)
      })
    },
    editOrg () {
      const params = Object.assign(this.orgForm,
        {
          is_disabled: this.orgForm.validate ? 0 : 1
        }
      )
      editOrg(params).then(res => {
        this.$message.success('修改成功')
        this.getOrgList()
        this.editCancel()
      }).catch(err => {
        this.$message.error(err.msg)
      })
    },
    deleteOrg (org_id) {
      const params = {
        org_id: org_id
      }
      deleteOrg(params).then(res => {
        this.$message.success('删除成功')
        this.getOrgList()
        this.fetchOrgTreeData({})
      }).catch(err => {
        this.$message.error(err.msg)
      })
    },
    editCancel () {
      this.$refs.orgFormRef.resetFields()
      this.orgForm = Object.assign(this.orgForm, orgForm)
      this.fetchOrgTreeData({})
      this.isShowEditModal = false
      this.confirmLoading = false
    },
    checkPhone (rule, value, callback) {
      if (!checkPhone(value)) {
        callback('手机号码有误，请重填！')
      } else {
        callback()
      }
    },
    checkPostCode (rule, value, callback) {
      if (!checkPostCode(value)) {
        callback('邮政编码有误，请重填！')
      } else {
        callback()
      }
    },
    reloadList () {
      this.getOrgList()
    },
    changeColumns (columns) {
      this.tableColumns = columns
    },
    handleScroll (e) {
      const top = this.$refs.tableRef.$el.offsetTop + 106 - e.target.scrollTop
      if (top < 0) {
        this.showSticky = true
      } else {
        this.showSticky = false
      }
    },
    handleTableScroll () {
      this.table = this.$refs.tableRef
      this.stickyTable = this.$refs.stickyTableRef
      const scrollLeft = this.table.$el.querySelector('.ant-table-body').scrollLeft
      this.stickyTable.$el.querySelector('.ant-table-body').scrollLeft = scrollLeft
    }
  },
  beforeDestroy () {
    this.resetAllData()
    this.tableList.removeEventListener('scroll', this.handleTableScroll)
  }
}
</script>

<style lang="less" scoped>
.employee-org {
  padding: 5px;
}
.content {
  margin: 10px;
  .left-card {
    border-right: 1px solid #e8e8e8;
  }
  .right-card {
    margin-left: -1px;
    border-left: 1px solid #e8e8e8;
  }
}
.employee-sys-table {
  background-color: white;
  padding: 5px 10px;
  .toolRow {
    height: 60px;
    align-items: center;
    .listTitle {
      padding: 0 10px;
      // font-weight: 600;
    }
    .toolsBtn {
      .ant-btn {
        margin-right: 10px;
      }
    }
  }
}
/deep/ .table-list > tr > th {
  padding: 10px 8px;
}
/deep/ .ant-table-tbody > tr > td {
  padding: 10px 8px;
}

/deep/ .ant-tree li .ant-tree-node-content-wrapper {
  height: auto;
  word-break: break-word;
  white-space: pre-wrap;
}
.content-row {
  background-color: #fff;
}
.sticky-table {
  display: block !important;
  position: fixed;
  top: 48px;
  overflow: hidden;
  z-index: 10;
  /deep/ .ant-table-placeholder {
    display: none;
  }
  ::-webkit-scrollbar {
    width: 0 !important;
  }
}
</style>