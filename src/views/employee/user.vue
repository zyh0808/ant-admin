  ...
  <div class="employee-user" @scroll="handleScroll($event)">
    ...
    <div class="content">
      ...
      <div class="employee-user-table">
        ...
        <a-table :columns="tableColumns" :data-source="userList" ref="tableRef" class="table-list" :rowKey="record =>record.user_id" :pagination="pagination" :loading="isfetchUserList" @change="handleTableChange" @showSizeChange="onShowSizeChange" :scroll="{x: 2140}">
          <span slot="locked" slot-scope="text, record">
            ...
            <a-switch :checked="record.locked" @change="changeLockUser(record)" />
          </span>
          ...
          <template slot="action" slot-scope="record">
            ...
            <a-divider type="vertical" />
            <a-button type="link" size="small" @click="showUserAuthModal(record)">
              用户权限
            </a-button>
            <a-divider type="vertical" />
            <a-popconfirm v-if="userList.length" title="确认删除吗?" cancelText="取消" okText="确认" @confirm="() => deleteUser(record)">
    ...
    <a-modal :title="title" :visible="isShowEditModal" :confirm-loading="confirmLoading" @ok="editOk" @cancel="editCancel" cancelText="取消" okText="确定" :width="600">
      <a-form-model ref="userFormRef" :model="userForm" :rules="userFormRules" :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }">
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="用户姓名" prop="username" ref="username">
              <a-input v-model="userForm.username" placeholder="请输入" :max-length="20" @blur="
          () => {
          ...
          <a-col :span="12">
            <a-form-model-item label="员工账号" prop="emp_id">
              <a-select v-model="userForm.emp_id" placeholder="请选择">
                <a-select-option v-for="item in employeeList" :key="item.key" :value="item.key">
                  {{item.title}}
                </a-select-option>
              </a-select>
            </a-form-model-item>
        ...
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="登陆账号" prop="login_name" ref="login_name">
              <a-input v-model="userForm.login_name" placeholder="请输入" :max-length="20" @blur="
          () => {
        ...
        " />
          ...
          <a-col :span="12">
            <a-form-model-item label="登陆密码" prop="pwd" ref="pwd">
              <a-input v-model="userForm.pwd" placeholder="请输入" :max-length="64" @blur="
          () => {
        ...
        <a-form-model-item label="备注" :label-col="{ span: 4 }" :wrapper-col="{ span: 16 }">
          <a-input v-model="userForm.memo" type="textarea" :max-length="200" placeholder="请输入" />
        </a-form-model-item>
      </a-form-model>
    </a-modal>
    <a-modal title="重置密码" :visible="isShowResetPswModal" :confirm-loading="resetLoading" @ok="resetOk" @cancel="resetCancel" cancelText="取消" okText="确定" :width="500">
      <a-form-model ref="userFormRef" :model="resetPswForm" :rules="resetPswFormRules" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
        ...
        <a-form-model-item label="新密码" prop="newPwd" ref="newPwd">
          <a-input-password v-model="resetPswForm.newPwd" placeholder="请输入" @blur="
          () => {
    ...
    <a-modal :title="userTitle" :visible="isShowUserAuthModal" @cancel="isShowUserAuthModal = false" :footer="null" :bodyStyle="{ height: '700px' }" cancelText=" 取消" okText="确定" :width="1000">
      <a-transfer class="tree-transfer" :titles="['用户权限列表（未选）', '用户权限列表（已选）']" :data-source="unselectUserAuth" :target-keys="userAuthTargetKeys" :render="item => item.title" :show-select-all="false" @change="userAuthChange">
        <template slot="children" slot-scope="{ props: { direction, selectedKeys }, on: { itemSelect } }">
          <a-tree v-if="direction === 'left'? (unselectUserAuth && unselectUserAuth.length > 0 ) : (selectUserAuth && selectUserAuth.length > 0 ) " blockNode checkable defaultExpandAll :checkedKeys="[...selectedKeys, ...userAuthTargetKeys]" :treeData="direction === 'left' ? unselectUserAuth : selectUserAuth" @check="
            (_, props) => {
