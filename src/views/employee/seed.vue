  ...
  <div class="employee-seed" @scroll="handleScroll($event)">
    ...
    <div class="content">
      ...
      <div class="employee-seed-table">
        ...
        <a-table :columns="tableColumns" ref="tableRef" :data-source="seedPageList" class="table-list" :rowKey="record => record.code_key" :loading="isfetchSeedPageList" :pagination="pagination" @change="handleTableChange" @showSizeChange="onShowSizeChange" :scroll="{x:1050}">
          <span slot="isDayUpdate" slot-scope="isDayUpdate">
            <!-- <a-switch :checked="isDayUpdate" disabled /> -->
            <a-tag v-if="isDayUpdate" color="green">是</a-tag>
          ...
          <template slot="action" slot-scope="record">
            ...
            <a-popconfirm v-if="seedPageList.length" title="确认删除吗?" cancelText="取消" okText="确认" @confirm="() => deleteSeed(record.code_key)">
    ...
    <a-modal :title="title" :visible="isShowEditModal" :confirm-loading="confirmLoading" @ok="editOk" @cancel="editCancel" cancelText="取消" okText="确定" :width="700">
      <a-form-model ref="seedFormRef" :model="seedForm" :rules="seedFormRules" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
        ...
        <a-form-model-item label="编码生成日期" prop="code_date">
          <a-date-picker v-model="seedForm.code_date" type="date" :format="dateFormat" allowClear placeholder="请选择" />
        </a-form-model-item>
