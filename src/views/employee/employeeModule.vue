<template>
  <div class="employee-module" @scroll="handleScroll($event)">
      ...
      <a-row class="content-row">
        <a-col :span="5">
          <a-card title="目录信息" :bordered="false" class="left-card">
            <a-tree v-if="moduleTreeData && moduleTreeData.length > 0"
              :default-expanded-keys="['TOP']" :tree-data="moduleTreeData"
              @select="onTreeSelect">
          ...
          <a-card title="区域信息" class="right-card" :bordered="false"
            ...
            <div class="employee-sys-table">
              ...
              <a-table :columns="tableColumns" ref="tableRef"
                :data-source="moduleList" class="table-list"
                :rowKey="record => record.module_id"
                :loading="isfetchModuleList" :pagination="pagination"
                ...
                <template slot="action" slot-scope="record">
                  ...
                  <a-button type="link" size="small"
                    @click="showAuthModal(record)">
                    页面权限
                  </a-button>
                  <a-divider type="vertical" />
                  <a-popconfirm v-if="moduleList.length" title="确认删除吗?"
                    cancelText="取消" okText="确认"
                    @confirm="() => deleteModule(record.module_id)">
                    <a-button type="link" size="small">
                      删除
                    </a-button>
                  </a-popconfirm>
    ...
    <a-modal :title="title" :visible="isShowEditModal"
      ...
      <a-form-model ref="moduleFormRef" :model="moduleForm"
        :rules="moduleFormRules" :label-col="{ span: 3 }"
        :wrapper-col="{ span: 20 }">
        ...
        <a-form-model-item label="图标" prop="icon" ref="icon">
          <a-input v-model="moduleForm.icon" placeholder="请输入" :max-length="100"
            @blur="
          () => {
        ...
        <a-row>
          ...
          <a-col :span="12">
            <a-form-model-item label="有效" :label-col="{ span: 6 }"
              :wrapper-col="{ span: 16 }">
        ...
        <a-form-model-item label="项目描述">
          <a-input v-model="moduleForm.memo" type="textarea"
            placeholder="请输入" />
        </a-form-model-item>
      </a-form-model>
    </a-modal>
    <a-modal :title="authTitle" :visible="isShowAuthModal" keyboard
      :footer="null" destroyOnClose @cancel="isShowAuthModal = false"
      :width="1000" :bodyStyle="{height: '700px'}">
      <a-transfer class="tree-transfer" :titles="['模块权限列表（未选）', '模块权限列表（已选）']"
        :data-source="unselectPageAuth" :target-keys="targetKeys"
        :render="item => item.title" :show-select-all="false"
        @change="onChange">
        <template slot="children"
          slot-scope="{ props: { direction, selectedKeys }, on: { itemSelect } }">
          <a-tree
            v-if="direction === 'left'? (unselectPageAuth && unselectPageAuth.length > 0 ) : (selectPageAuth && selectPageAuth.length > 0 ) "
            blockNode checkable defaultExpandAll
            :checkedKeys="[...selectedKeys, ...targetKeys]"
            :treeData="direction === 'left' ? unselectPageAuth : selectPageAuth"
            @check="
            (_, props) => {
