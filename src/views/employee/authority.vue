  ...
  <div class="employee-auth" @scroll="handleScroll($event)">
    ...
    <div class="content">
      ...
      <div class="employee-auth-table">
        ...
        <a-table :columns="tableColumns" ref="tableRef" :data-source="authPageList" class="table-list" :rowKey="record => record.auth_id" :loading="isfetchAuthPageList" :pagination="pagination" @change="handleTableChange" @showSizeChange="onShowSizeChange" :scroll="{x:1580}">
          ...
          <template slot="action" slot-scope="record">
            ...
            <a-popconfirm v-if="authPageList.length" title="确认删除吗?" cancelText="取消" okText="确认" @confirm="() => deleteAuth(record.auth_id)">
        ...
        <a-table :columns="tableColumns" :class="showSticky?'sticky-table'  : ''" ref="stickyTableRef" :style="{display: 'none',width: stickyWidth + 'px'}" table-layout="fixed" :scroll="{x:1580}">
        </a-table>
      </div>
    </div>
    <a-modal :title="title" :visible="isShowEditModal" :confirm-loading="confirmLoading" @ok="editOk" @cancel="editCancel" cancelText="取消" okText="确定" :width="800">
      <a-form-model ref="authFormRef" :model="authForm" :rules="authFormRules" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
        <a-row>
          <a-col :span="12">
        ...
        <a-form-model-item ref="btn_icon" label="按钮图标" prop="btn_icon" :label-col="{ span: 3 }" :wrapper-col="{ span: 16 }">
          <a-input v-model="authForm.btn_icon" placeholder="请输入" :max-length="100" @blur="
          () => {
        ...
        <a-form-model-item label="备注" :label-col="{ span: 3 }" :wrapper-col="{ span: 16 }" prop="memo">
