<template>
  <div class="employee-employee" @scroll="handleScroll($event)">
      ...
      <a-row class="content-row">
        <a-col :span="5">
          <a-card title="组织机构目录" class="left-card" :bordered="false">
            <a-tree v-if="orgTreeData && orgTreeData.length > 0" show-icon :default-expanded-keys="['0']" :tree-data="orgTreeData" @select="onTreeSelect">
        ...
        <a-col :span="19" style="height:100%">
          <a-card title="员工信息" class="right-card" ref="contentRowRefs" :bordered="false">
            <div class="employee-sys-table">
                ...
                <a-row class="toolsBtn">
                  <a-space :size="12">
                    <a-tooltip placement="top">
                      ...
                      <a-button type="primary" icon="plus-circle" @click="showEditModal()" :disabled="!addable" />
                    </a-tooltip>
                    <a-tooltip placement="top">
                      <template slot="title">
                        <span>刷新</span>
              ...
              <a-table :columns="tableColumns" ref="tableRef" :data-source="employeeList" class="table-list" :rowKey="record => record.emp_code" :pagination="pagination" :loading="isfetchEmployeeList" @change="handleTableChange" @showSizeChange="onShowSizeChange" :scroll="{x:950}">
                <span slot="validate" slot-scope="text, record">
                  ...
                  <a-switch :checked="record.validate" @change="changeDisableEmployee(record)" />
                </span>
                <template slot="action" slot-scope="record">
                  ...
                  <a-popconfirm v-if="employeeList.length" title="确认删除吗?" cancelText="取消" okText="确认" @confirm="() => deleteEmployee(record)">
    ...
    <a-modal :title="title" :visible="isShowEditModal" :confirm-loading="confirmLoading" @ok="editOk" @cancel="editCancel" cancelText="取消" okText="确定" :width="800">
      <a-form-model ref="employeeFormRef" :model="employeeForm" :rules="employeeFormRules" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
        <a-row>
          <a-col :span="12">
        ...
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="员工工号" prop="emp_code" ref="emp_code">
              <a-input v-model="employeeForm.emp_code" :max-length="10" placeholder="请输入" @blur="
          () => {
          ...
          <a-col :span="12">
            <a-form-model-item label="员工姓名" prop="emp_name" ref="emp_name">
              <a-input v-model="employeeForm.emp_name" :max-length="30" placeholder="请输入" @blur="
          () => {
        ...
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="岗位职务" prop="duty" ref="duty">
              <a-input v-model="employeeForm.duty" placeholder="请输入" :max-length="10" @blur="
          () => {
        ...
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="生日" prop="birthdayDate">
              <a-date-picker v-model="employeeForm.birthdayDate" type="date" :default-value="moment('2000-01-01', dateFormat)" :format="dateFormat" allowClear placeholder="请选择" />
            </a-form-model-item>
          </a-col>
          <a-col :span="12">
        ...
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="办公电话" prop="phone" ref="phone">
              <a-input v-model="employeeForm.phone" placeholder="请输入" :max-length="50" @blur="
          () => {
          ...
          <a-col :span="12">
            <a-form-model-item label="电子邮箱" prop="email" ref="email">
              <a-input v-model="employeeForm.email" placeholder="请输入" :max-length="50" @blur="
          () => {
        ...
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="办公传真">
              <a-input v-model="employeeForm.fax" placeholder="请输入" :max-length="50" />
            </a-form-model-item>
          </a-col>
          <a-col :span="12">
            <a-form-model-item label="邮政编码">
              <a-input v-model="employeeForm.postcode" placeholder="请输入" :max-length="6" />
            </a-form-model-item>
          </a-col>
        </a-row>
        <a-form-model-item label="办公地址" prop="addr" ref="addr" :label-col="{ span: 3 }" :wrapper-col="{ span: 16 }">
          <a-input v-model="employeeForm.addr" placeholder="请输入" :max-length="200" @blur="
          () => {
        ...
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="入职时间" prop="entryDate">
              <a-date-picker v-model="employeeForm.entryDate" type="date" :default-value="moment('2020-01-01', dateFormat)" :format="dateFormat" allowClear placeholder="请选择" />
            </a-form-model-item>
          </a-col>
          <a-col :span="12">
            <a-form-model-item label="QQ">
              <a-input v-model="employeeForm.qq" placeholder="请输入" :max-length="20" />
            </a-form-model-item>
          </a-col>
        </a-row>
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="微信">
              <a-input v-model="employeeForm.wechat" placeholder="请输入" :max-length="30" />
            </a-form-model-item>
          </a-col>
          <a-col :span="12">
        ...
        <a-form-model-item label="备注" :label-col="{ span: 3 }" :wrapper-col="{ span: 16 }">
          <a-input v-model="employeeForm.memo" type="textarea" placeholder="请输入" />
        </a-form-model-item>
      </a-form-model>
    </a-modal>
  </div>
